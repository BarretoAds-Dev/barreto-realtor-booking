...
async function runHealthCheck() {
  ...
  console.log(`${colors.blue}║   🏥 Health Check del Sistema        ║${colors.reset}`);
  console.log(`${colors.blue}╚════════════════════════════════════════╝${colors.reset}\n`);
  ...
  if (checks.failed === 0) {
    console.log(`${colors.green}✅ Sistema saludable - Todo funcionando correctamente!${colors.reset}`);
    process.exit(0);
  } else {
    console.log(`${colors.red}❌ Sistema con problemas - Revisa los errores arriba${colors.reset}`);
    process.exit(1);
  }
}
...
runHealthCheck().catch((error) => {
  console.error(`${colors.red}❌ Error fatal en health check:${colors.reset}`, error);
  process.exit(1);
});
