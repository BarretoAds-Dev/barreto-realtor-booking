...
import { EasyBrokerServiceAPI } from '../../../../../core/services/easybroker.service';
import { validateEasyBrokerConfig } from '../../../../../core/config/easybroker';
...
export const GET: APIRoute = async ({ params }) => {
	try {
		...
		try {
			const apiKey = import.meta.env.EASYBROKER_API_KEY || import.meta.env.PUBLIC_EASYBROKER_API_KEY || '';
			...
			console.error('❌ Error al obtener propiedad de Easy Broker:', fetchError);
		...
		return new Response(html, {
			...
			headers: {
				...
				'Content-Disposition': `attachment; filename="ficha-${property.public_id || publicId}.html"`,
			},
		});
	} catch (error) {
...
<body>
	<div class="header">
		...
		${price ? `<div class="price">${escapeHtml(price.formatted_amount)} ${price.currency || 'MXN'}</div>` : ''}
		<div class="info-value">
			📍 ${escapeHtml(formatLocation(location))}
		</div>
	</div>

	${property.title_image_full ? `<img src="${escapeHtml(property.title_image_full)}" alt="${escapeHtml(property.title)}" class="image">` : ''}
	...
	<div class="section">
		...
		<div class="info-value">
			${property.tags.map((tag: string) => `<span style="display: inline-block; background: #e2e8f0; padding: 5px 10px; border-radius: 5px; margin: 5px;">${escapeHtml(tag)}</span>`).join('')}
		</div>
	</div>
	` : ''}
	...
	<div class="section">
		...
		<div class="info-value">
			...
			${location?.neighborhood ? `<p><strong>Colonia:</strong> ${escapeHtml(location.neighborhood)}</p>` : ''}
			${location?.address ? `<p><strong>Dirección:</strong> ${escapeHtml(location.address)}</p>` : ''}
			${location?.postal_code ? `<p><strong>Código Postal:</strong> ${escapeHtml(location.postal_code)}</p>` : ''}
		</div>
	</div>

	<div style="margin-top: 40px; padding-top: 20px; border-top: 2px solid #e2e8f0; text-align: center; color: #64748b; font-size: 12px;">
		<p>Ficha generada el ${new Date().toLocaleDateString('es-ES', { year: 'numeric', month: 'long', day: 'numeric' })}</p>
