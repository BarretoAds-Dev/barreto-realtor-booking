...
export const GET: APIRoute = async ({ request, url }) => {
	try {
		...
		if (!validateEasyBrokerConfig()) {
			return new Response(
				JSON.stringify({
					...
					message: 'EASYBROKER_API_KEY no está configurada en las variables de entorno',
				}),
				{
					status: 500,
		...
		// Obtener propiedades
		console.log('📡 API: Obteniendo propiedades de Easy Broker con filtros:', filters);
		...
		console.log(
			`✅ API: ${result.data.content?.length || 0} propiedades obtenidas de Easy Broker`
		);
