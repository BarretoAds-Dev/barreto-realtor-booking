			...
			if (error.code === 'PGRST205' || error.message?.includes('Could not find the table')) {
				...
				return new Response(
					JSON.stringify({
						...
						details: 'Por favor, ejecuta el script SQL en Supabase para crear la tabla. Ver archivo PROPERTIES_TABLE_SETUP.sql',
...
export const GET: APIRoute = async () => {
	try {
		...
		if (error) {
			...
			if (error.code === 'PGRST205' || error.message?.includes('Could not find the table')) {
				console.warn('⚠️ Tabla properties no existe en Supabase. Retornando array vacío.');
