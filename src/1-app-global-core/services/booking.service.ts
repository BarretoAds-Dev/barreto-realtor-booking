 ...
 *
 * Unifica la gestión de citas (appointments) y disponibilidad (availability)
 * en un solo servicio cohesivo para simplificar el código y mejorar el mantenimiento.
 *
 * @example
 * ```ts
 * import { BookingService } from '@/1-app-global-core/services';
 *
 ...
 */
async function checkSlotAvailability(slotId: string): Promise<{
  available: boolean;
  bookedCount: number;
  capacity: number;
  error: Error | null;
 ...
 */
async function updateSlotBookedCount(slotId: string): Promise<void> {
  try {
    // Usar cliente admin obligatoriamente para bypass RLS en operaciones del servidor
    const client = getSupabaseAdmin();
...
async function updateAppointment(
  ...
  try {
    ...
    if (slotChanged) {
      await updateSlotBookedCount(existingAppointment.slot_id);
      await updateSlotBookedCount(slot.id);
    } else {
...
// ============================================================================
// Public API - BookingService
// ============================================================================

/**
 * Servicio unificado de reservas
 * Combina funcionalidades de disponibilidad y citas
 */
...
/**
 * @deprecated Usar BookingService en su lugar
 * Mantenido para compatibilidad hacia atrás
 */
export class AppointmentsService {
  static normalizeTime = normalizeTime;
  static findAvailableSlot = findAvailableSlot;
  static checkSlotAvailability = checkSlotAvailability;
  static createAppointment = createAppointment;
  static updateAppointment = updateAppointment;
  static updateSlotBookedCount = updateSlotBookedCount;
}
...
export class AvailabilityService {
  static getDayOfWeek = getDayOfWeek;
  static getAvailabilitySlots = getAvailabilitySlots;
}
