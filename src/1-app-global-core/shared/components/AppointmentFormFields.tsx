...
interface AppointmentFormFieldsProps {
	...
	onBudgetChange?: (fieldName: 'budgetRentar' | 'budgetComprar', value: string) => void; // Callback para validación en tiempo real
...
export default function AppointmentFormFields({
	...
	const handleBudgetChange = (fieldName: 'budgetRentar' | 'budgetComprar', value: string) => {
		if (onBudgetChange) {
					...
					>
						...
						>
							...
							{BUDGET_OPTIONS_RENTAR.map((opt) => {
								const isValid = !propertyPrice || propertyPrice === 0 || opt.min >= propertyPrice;
								...
								return (
									<option
										...
										value={opt.value}
										disabled={isDisabled}
										style={isDisabled ? { color: '#9ca3af', backgroundColor: '#f9fafb' } : { color: '#111827', backgroundColor: 'white' }}
									>
										{opt.label}
										{isDisabled && ` (Menor al precio: ${new Intl.NumberFormat('es-MX', { style: 'currency', currency: 'MXN' }).format(propertyPrice)})`}
									</option>
								);
							})}
						</select>
						...
						{propertyPrice > 0 && (
							<p class="mt-1 text-xs text-gray-500">
								Precio de la propiedad: {new Intl.NumberFormat('es-MX', { style: 'currency', currency: 'MXN' }).format(propertyPrice)}
							</p>
						)}
					</FormField>
