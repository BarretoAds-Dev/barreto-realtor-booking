/**
 * Utilidades de Validación (Validators)
 *
 * Centraliza todas las funciones de validación del sistema.
 * Validación del lado del cliente sin dependencias de Zod.
 *
 * @example
 * ```ts
 * import { validateName, validateEmail, validateAppointmentClient } from '@/1-app-global-core/utils';
 * ```
 */
...
export function validateAppointmentClient(data: any): ValidationResult {
  ...
  if (
    !data.operationType ||
    (data.operationType !== 'rentar' && data.operationType !== 'comprar')
  ) {
    ...
    if (data.resourceType === 'credito-bancario') {
      ...
      if (!data.creditoPreaprobado) {
        errors.creditoPreaprobado =
          '¿Cuenta con crédito preaprobado? es requerido';
      }
