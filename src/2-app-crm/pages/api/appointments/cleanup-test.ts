...
export const POST: APIRoute = async ({ request }) => {
	try {
		...
		if (import.meta.env.PROD) {
			return new Response(
				JSON.stringify({ error: 'Esta función solo está disponible en desarrollo' }),
		...
		if (fetchError) {
			return new Response(
				JSON.stringify({ error: 'Error al obtener citas', details: fetchError.message }),
				{ status: 500, headers: { 'Content-Type': 'application/json' } }
			);
		}
		...
		const testAppointments = appointments?.filter(apt => {
			...
			const isTestEmail = apt.email?.toLowerCase().includes(emailPattern.toLowerCase());
		...
		if (deleteError) {
			return new Response(
				JSON.stringify({ error: 'Error al eliminar citas', details: deleteError.message }),
				{ status: 500, headers: { 'Content-Type': 'application/json' } }
			);
		}

		// Actualizar contadores de slots afectados
		const affectedSlotIds = [...new Set(testAppointments.map(apt => apt.slot_id))];
