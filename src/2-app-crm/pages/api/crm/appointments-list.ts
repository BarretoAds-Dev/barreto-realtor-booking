...
import type { Appointment } from '@/1-app-global-core/core/types/appointment';
import { getEasyBrokerApiKey, getEasyBrokerBaseUrl } from '@/1-app-global-core/core/config/easybroker';
	...
	try {
				...
				const appointmentsWithProperties = await Promise.all(
					(fallbackData || []).map(async (apt: any) => {
						if (apt.property_id) {
							try {
								const { data: propertyData } = await supabase
									.from('properties')
									.select('id, title, address, price, property_type, bedrooms, bathrooms, area, features')
									...
									try {
										...
										if (apiKey && baseUrl) {
											...
											if (easyBrokerRes.ok) {
												...
												imageUrl = ebData.title_image_thumb || ebData.title_image_full || ebData.images?.[0]?.url || null;
											}
										}
									} catch (e) {
						...
						// Si no hay property_id pero hay notas con información de Easy Broker, intentar extraer
							...
							const easyBrokerIdMatch = apt.notes.match(/Easy Broker ID:\s*([^\s\n]+)/i);
							const imageMatch = apt.notes.match(/Imagen:\s*([^\s\n]+)/i);
							const propertyTitleMatch = apt.notes.match(/Propiedad:\s*([^\n]+)/i);
		...
		const formattedAppointments = await Promise.all(
			appointments.map(async (apt: any) => {
				...
				if (apt.properties && !apt.properties.imageUrl && !imageUrl && apt.notes) {
					const imageMatch = apt.notes.match(/Imagen:\s*([^\s\n]+)/i);
					if (imageMatch) {
						imageUrl = imageMatch[1].trim();
					}
