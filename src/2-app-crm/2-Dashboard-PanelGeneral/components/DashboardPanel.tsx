...
export default function DashboardPanel({
    ...
    const loadProperties = async () => {
      try {
        ...
        if (supabaseResponse.ok) {
          ...
          supabaseCount =
            supabaseData.properties?.filter(
              (prop: any) => prop.status === 'active' || !prop.status
            ).length || 0;
        }
  ...
  const weeklyData = useMemo(() => {
    ...
    const dayCounts = days.map((day, index) => {
      ...
      const newCount = appointments.filter((apt) => {
        ...
        try {
          ...
          const matches =
            createdDateNormalized.getTime() === dayDateNormalized.getTime();
          ...
          if (index === 0 && matches) {
            console.log('✅ Match found for today:', {
              ...
              createdDateNormalized:
                createdDateNormalized.toLocaleDateString('es-ES'),
    ...
    if (todayIndex >= 0) {
      ...
      console.log(
        `📈 Today's counts: New=${dayCounts[todayIndex].newCount}, Cancelled=${dayCounts[todayIndex].cancelledCount}`
      );
    }
        ...
        {metrics.map((metric, index) => (
          ...
          >
            <div class="flex items-start justify-between mb-3">
              ...
              <div
                class={`text-xs sm:text-sm font-semibold flex items-center gap-1 ${
      ...
      <div class="grid grid-cols-1 lg:grid-cols-2 gap-4 sm:gap-5 md:gap-6">
        ...
        <div class="bg-gray-50 rounded-xl p-4 sm:p-5 md:p-6 border border-gray-200 shadow-sm">
          ...
          <div class="relative h-48 sm:h-56 md:h-64">
            ...
            {/* Etiquetas de días */}
            <div
              class="absolute bottom-0 left-0 right-0 flex justify-between px-2"
