...
export default function Calendar({
  ...
  const hasAvailableSlots = (date: Date): boolean => {
    ...
    return daySlots.slots.some(
      (slot) => slot.available && slot.booked < slot.capacity
    );
  };

  const getDayAvailabilityStatus = (
    date: Date
  ): 'available' | 'half-full' | 'full' | 'none' => {
    const dateStr = formatDateLocal(date);
    const daySlots = availableSlots.find((slot) => slot.date === dateStr);
    if (!daySlots || daySlots.slots.length === 0) return 'none';

    const enabledSlots = daySlots.slots.filter(
      (slot) => slot.enabled !== false
    );
    if (enabledSlots.length === 0) return 'none';

    const totalCapacity = enabledSlots.reduce(
      (sum, slot) => sum + slot.capacity,
      0
    );
    const totalBooked = enabledSlots.reduce(
    ...
    const hasAvailable = enabledSlots.some(
  ...
  return (
    <div class="max-w-md mx-auto transition-all duration-500">
      {/* Mostrar información del inmueble si está disponible */}
      {selectedProperty && (
          ...
          <div class="flex items-start gap-3">
            ...
            <div class="flex-1">
              <p class="text-sm font-semibold text-yellow-800 mb-1">
                Las citas deben tener 24 horas de anticipación
              </p>
              <p class="text-xs text-yellow-700">
                Por favor selecciona una cita para el día siguiente o posterior.
              </p>
            </div>
            <button
      ...
      <div class="flex items-center justify-between mb-6 bg-white p-3 border border-gray-200 rounded-lg shadow-md">
        ...
        >
          <svg
            class="w-6 h-6 text-gray-600 hover:text-gray-900 transition-colors"
            fill="none"
            stroke="currentColor"
            viewBox="0 0 24 24"
          >
              ...
              class={`
                  ...
                  availabilityStatus !== 'none'
                    ...
                    : ''
								...
								${isTodayDate && isSelected ? 'bg-gray-900 text-white' : ''}
							`}
              title={
                isTodayDate
