...
export default function BookingPublic({
  ...
  // Cargar propiedad desde URL si existe propertyId (solo para formulario público)
  useEffect(() => {
    if (isModal || useCRMForm || preselectedProperty) return;

    const urlParams = new URLSearchParams(window.location.search);
    const propertyIdFromUrl = urlParams.get('propertyId');

    if (!propertyIdFromUrl || selectedProperty) return;

    const loadPropertyFromUrl = async (): Promise<void> => {
      setIsLoadingProperty(true);
      try {
        // Intentar cargar desde EasyBroker primero
        const response = await fetch(
          `/api/easybroker/properties/${propertyIdFromUrl}`
        );
        ...
        if (response.ok) {
          ...
          if (property) {
            ...
            setSelectedProperty({
              id: property.public_id || property.id,
        ...
        console.warn('Error al cargar propiedad desde URL:', error);
      } finally {
    ...
    loadPropertyFromUrl();
  }, [isModal, useCRMForm, preselectedProperty, selectedProperty]);
