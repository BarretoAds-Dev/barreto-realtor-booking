...
import { validateAppointmentClient } from '../utils/validation';
...
export function useAppointmentForm({ selectedDate, selectedTime, formId }: UseAppointmentFormProps) {
	...
	const validateForm = async (): Promise<{ success: boolean; data?: any; errors?: Record<string, string> }> => {
		const form = document.getElementById(formId) as HTMLFormElement;
		if (!form) {
			return { success: false, errors: { general: 'Formulario no encontrado' } };
		}
